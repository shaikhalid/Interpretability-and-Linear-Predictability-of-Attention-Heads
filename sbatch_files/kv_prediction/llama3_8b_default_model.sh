...
# TRAINING LR ON LANGUAGE MODELING
DATASET="eli5"
...
# EVALUATION LR ON DOWNSTREAM TASKS
DATASET="winogrande"
TASK_TYPE="multiple_choice"

# baseline
python main.py --model_type ${MODEL_FAMILY} --ckpt_dir ${MODEL} --few_shot_number 0 --dataset ${DATASET} --batch_size ${BATCH_SIZE} --task_type ${TASK_TYPE}  --context_length ${CONTEXT_LENGTH} > sbatch_files/kv_prediction/logs/$(echo Baseline_${DATASET}_${ALIAS} | tr '/' '_').log

# run eval with kv prediction
python main.py --model_type ${MODEL_FAMILY} --ckpt_dir ${MODEL} --few_shot_number 0 --dataset ${DATASET} --batch_size ${BATCH_SIZE} --task_type ${TASK_TYPE}  --context_length ${CONTEXT_LENGTH} --predict True --predict_keys True --predict_values True --alias ${ALIAS} --default_prediction_model 1 > sbatch_files/kv_prediction/logs/$(echo Prediction_${DATASET}_${ALIAS} | tr '/' '_').log 
