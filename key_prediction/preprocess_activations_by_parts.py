...
# --- NEW: Token Part Filtering ---
def filter_tokens_by_part(df, part_number):
    """Filters the DataFrame to keep only tokens within a specific quartile (part) for each sample.

    Args:
        df (pd.DataFrame): The expanded DataFrame with token-level data.
        part_number (int): The part number (1, 2, 3, or 4) representing the quartile.

    Returns:
        pd.DataFrame: Filtered DataFrame containing only tokens from the specified part.
    """
    if part_number is None:
        logging.info("No --part specified, keeping all tokens.")
        return df

    if part_number not in [1, 2, 3, 4]:
        raise ValueError("--part must be 1, 2, 3, or 4.")

    start_time = time.time()
    logging.info(f"Starting token filtering for part {part_number}/4...")

    filtered_dfs = []
    # Group by sample number to process each sample independently
    grouped_by_sample = df.groupby('sample_number')

    for sample_num, sample_df in grouped_by_sample:
        if sample_df.empty:
            continue

        # Find the maximum token position for this specific sample
        max_token_pos = sample_df['token_position'].max()
        total_tokens = max_token_pos + 1 # Since token_position is 0-indexed

        # Calculate the size of each part (quartile)
        # Use math.ceil to handle cases where total_tokens is not divisible by 4
        part_size = math.ceil(total_tokens / 4)

        # Determine the start and end token positions for the requested part
        start_pos = (part_number - 1) * part_size
        # Ensure end_pos doesn't exceed the actual max position
        end_pos = min(start_pos + part_size, total_tokens) # Exclusive upper bound for slicing

        # Filter tokens within the calculated range [start_pos, end_pos)
        part_df = sample_df[(sample_df['token_position'] >= start_pos) & (sample_df['token_position'] < end_pos)].copy()
        filtered_dfs.append(part_df)

    if not filtered_dfs:
        logging.warning("Token part filtering resulted in an empty DataFrame.")
        # Return an empty DataFrame with the same columns
        return pd.DataFrame(columns=df.columns)

    final_df = pd.concat(filtered_dfs, ignore_index=True)
    # Sort again as concatenation might change order
    final_df = final_df.sort_values(by=['sample_number', 'layer_id', 'head_id', 'token_position'], ignore_index=True)

    logging.info(f"Token part filtering complete. Kept part {part_number}. Original rows: {len(df)}, Filtered rows: {len(final_df)}")
    logging.info(f"Token part filtering time: {time.time() - start_time:.2f} seconds")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--matrix_type", type=str, default="k_matrix", choices=["k_matrix", "v_matrix"],
    ...
    parser.add_argument("--part", type=int, default=None, choices=[1, 2, 3, 4],
                        help="Keep only tokens from a specific quartile (1=0-25%, 2=25-50%, 3=50-75%, 4=75-100%) for each sample.")
    ...
    try:
        ...
        # --- NEW Step 3.7: Filter Tokens by Part ---
        if args.part is not None:
            expanded_df = filter_tokens_by_part(expanded_df, args.part)
            if expanded_df.empty:
                 logging.error(f"DataFrame is empty after filtering for part {args.part}. Cannot proceed.")
                 exit(1)
