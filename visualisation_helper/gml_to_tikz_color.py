...
LAYER_SPACING = 0.5 # Horizontal spacing between layers
NODE_SPACING = 0.8  # Vertical spacing between nodes within a layer
TITLE_Y_OFFSET = 0.5 # Vertical offset for layer titles above nodes
...
def get_layer_style(layer, min_layer, max_layer):
    """Determines the style of a node based on its layer position with progressive color density."""
    num_layers = max_layer - min_layer + 1
    layer_section_size = num_layers / 3  # Divide into 3 equal parts
    
    # Calculate relative position within each section (0.0 to 1.0)
    if layer <= min_layer + layer_section_size:
        # Early section (NOW orange)
        section = "early"
        start_of_section = min_layer
        section_size = layer_section_size
        color_name = "orange" # Changed from cyan
    elif layer <= min_layer + 2 * layer_section_size:
        # Middle section (NOW cyan)
        section = "middle"
        start_of_section = min_layer + layer_section_size
        section_size = layer_section_size
        color_name = "cyan"   # Changed from orange
    else:
        # Late section (magenta - unchanged)
        section = "late"
        start_of_section = min_layer + 2 * layer_section_size
        section_size = layer_section_size
        color_name = "magenta"
    
    # Calculate relative position in section (0 to 1)
    rel_pos = (layer - start_of_section) / section_size if section_size > 0 else 0
    
    # Calculate color intensity (30% to 80%)
    intensity = 30 + int(50 * rel_pos)
    
    # Create a dynamic style directly in the node - re-added 'draw' option
    return f"circle, draw, fill={color_name}!{intensity}, minimum size=8pt, inner sep=1pt"
...
def main():
    ...
    # 1. Pre-calculate node styles and colors
    node_styles = {}
    node_colors = {}
    for node_id in G.nodes():
        data = node_data[node_id]
        layer = get_node_layer(data, args.layer_attr)
        style = get_layer_style(layer, min_layer, max_layer)
    ...
    for node_id, (x, y) in positions.items():
        style = node_styles.get(node_id, "circle, fill=gray") # Use pre-calculated style
        name = sanitize_name(str(node_id))
        lines.append(fr"    \node[{style}] ({name}) at ({x:.2f},{y:.2f}) {{}}; % Node defined here")
    ...
    for u, v, edge_data in G.edges(data=True):
        ...
        lines.append(fr"    \path[draw, color={source_color}, -latex] ({name_u}) edge ({name_v});")
        drawn_edges += 1
    lines.append(r"  \end{pgfonlayer}")
