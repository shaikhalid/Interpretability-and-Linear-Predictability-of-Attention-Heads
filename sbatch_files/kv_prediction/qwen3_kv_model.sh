...
if [ $# -lt 2 ]; then
    ...
    echo "Example: $0 winogrande multiple_choice"
    exit 1
fi
...
python main.py --model_type ${MODEL_FAMILY} --ckpt_dir ${MODEL} --few_shot_number 0 --dataset ${DATASET} --batch_size 8 --task_type ${TASK_TYPE}  --context_length ${CONTEXT_LENGTH} > sbatch_files/kv_prediction/logs/$(echo Baseline_${DATASET}_${ALIAS} | tr '/' '_').log 2>&1
